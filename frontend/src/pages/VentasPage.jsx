              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-4">
                      ...
                      <ul className="absolute z-10 w-full bg-white border border-stone/20 rounded-lg mt-1 shadow-lg max-h-48 sm:max-h-64 overflow-y-auto">
                        {resultados.map((producto) => (
                          <li
                            ...
                            onMouseEnter={() => setProductoPreview(producto)}
                            className="flex items-center gap-3 px-3 sm:px-4 py-2 cursor-pointer hover:bg-cream"
                          >
                            <ImagenProducto
                              url={producto.url_imagen}
                              nombre={producto.nombre}
                              className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg border border-stone/20 shrink-0"
                              fallbackText="📦"
                            />
                            <div className="flex-1 min-w-0">
                              <div className="text-sm sm:text-base text-ink truncate">
                                {producto.nombre}
                              </div>
                            ...
                            <span className="font-mono text-xs sm:text-sm text-primary font-semibold shrink-0">
                          ...
                          {carrito.map((it) => (
                            ...
                            >
                              ...
                              <td className="px-2 sm:px-4 py-2 sm:py-3">
                                <button
                                  onClick={() =>
                                    quitarDelCarrito(it.id_producto)
                                  }
                    ...
                    <div className="w-full sm:w-auto">
                      ...
                      {clienteSeleccionado && (
                        <p className="text-xs sm:text-sm text-success mt-1">
                          Cliente: {clienteSeleccionado.nombre} - Se sumarán
                          puntos
                        </p>
                      )}
                    </div>
                ...
                <div className="hidden lg:block">
                  <div className="sticky top-4 bg-white rounded-lg shadow p-4">
                    <p className="font-display font-semibold text-ink mb-3 text-sm">
                      Vista previa
                    </p>
                    {productoPreview ? (
                      <>
                        <ImagenProducto
                          url={productoPreview.url_imagen}
                          nombre={productoPreview.nombre}
                          className="w-full aspect-square rounded-lg border border-stone/20"
                          fallbackText="📦"
                        />
                        <p className="mt-3 text-sm font-semibold text-ink truncate">
                          {productoPreview.nombre}
                        </p>
                        <p className="text-xs text-stone">
                          Stock: {productoPreview.stock}
                        </p>
                        <p className="font-mono text-primary font-semibold mt-1">
                          Bs {Number(productoPreview.precio_venta).toFixed(2)}
                        </p>
                      </>
                    ) : (
                      <div className="aspect-square rounded-lg bg-cream flex items-center justify-center text-stone text-xs text-center px-3">
                        Busca o selecciona un producto para ver su imagen aquí
                      </div>
                    )}
                  </div>
                </div>
                          ...
                          >
                            ...
                            <td className="px-2 sm:px-4 py-2 sm:py-3 text-stone text-xs sm:text-sm">
                              {venta.detalles.map((d) => (
                                ...
                                >
                                  <ImagenProducto
                                    url={d.producto?.url_imagen}
                                    nombre={d.producto?.nombre}
                                    className="w-6 h-6 rounded border border-stone/20 shrink-0"
                                    fallbackText="📦"
                                  />
                                  <span>
                                    {d.producto?.nombre ??
